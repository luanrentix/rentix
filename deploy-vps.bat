...
"%GIT_BASH%" -lc "ssh -i %SSH_KEY% %VPS_USER%@%VPS_HOST% 'cd %VPS_PATH% && git pull && docker compose -f docker-compose.prod.yml --env-file .env.production up -d --build && docker image prune -f && docker builder prune -f && docker compose -f docker-compose.prod.yml ps && curl -I https://www.contrx.com.br && curl -I https://api.contrx.com.br'"
if errorlevel 1 (
